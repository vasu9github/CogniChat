...
const ChatInterface = ({ selectedChatId, setSelectedChatId, setChats }) => {
  // All your state and logic can remain the same
  const { user } = useAuth();
  const [messages, setMessages] = useState([]);
  const [input, setInput] = useState('');
  const [loading, setLoading] = useState(false);
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const res = await axios.post('https://cognichat-backend.onrender.com/api/chats/generate', { prompt: currentInput, chatId: selectedChatId }, { withCredentials: true });
      const aiMessage = { role: 'model', content: res.data.aiResponse };
      setMessages((prev) => [...prev, aiMessage]);
      if (!selectedChatId && res.data.chat) {
        setSelectedChatId(res.data.chat._id);
  ...
  return (
    // --- FIX: Added relative z-10 and changed height/overflow for sticky nav ---
    <div className='relative z-10 flex flex-col flex-1 min-h-0 bg-black'>
      {/* This container will now scroll independently of the sticky navbar */}
      <div className='flex-1 p-6 space-y-4 overflow-y-auto'>
        {messages.length === 0 && !loading ? (
          <div className='text-center h-full flex flex-col items-center justify-center'>
            <img src='/logo.png' alt='CogniChat Logo' width={66} height={66} />
      ...
      <div className='p-4 bg-black border-t border-gray-800'>
        <form onSubmit={handleSendMessage} className='relative max-w-4xl mx-auto flex items-end'>
          <Textarea
            ...
            minRows={1}
            maxRows={5}
            placeholder={user ? 'Ask CogniChat anything...' : 'Please sign in to start chatting.'}
            className='w-full bg-gray-900 border border-gray-700 rounded-2xl pl-4 pr-12 py-3 text-gray-100 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:bg-gray-800'
            disabled={!user || loading}
          />
          <button
            type='submit'
            disabled={!input.trim() || !user || loading}
            className='absolute right-2 bottom-2 p-2 rounded-full bg-blue-500 hover:bg-blue-600 transition-colors disabled:bg-gray-600 disabled:cursor-not-allowed'
          >
            <Send size={20} className='text-white' />
          </button>
        </form>
