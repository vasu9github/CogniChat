...
const Sidebar = ({ isOpen, toggleSidebar, setSelectedChatId, startNewChat, chats, setChats }) => {
  ...
  useEffect(() => {
    const fetchChats = async () => {
      if (user) {
        try {
          // FIX: Added the correct '/api/chats' path
          const res = await axios.get(`${API_BASE_URL}/api/chats`, { withCredentials: true });
          setChats(res.data);
        } catch (error) {
          console.error("Failed to fetch chats", error);
        }
  ...
  const handleDelete = async (e, chatId) => {
    ...
    if (window.confirm("Are you sure you want to delete this chat?")) {
      try {
        // FIX: Added the correct '/api/chats/:chatId' path
  ...
  return (
    <>
      {isOpen && <div onClick={toggleSidebar} className="fixed inset-0 bg-black/60 z-10 md:hidden"></div>}
      <aside
        className={`fixed md:relative z-20 flex flex-col bg-black h-full border-r border-gray-700 transition-transform duration-300 ease-in-out
                   ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}
      >
        <div className="w-64 h-full flex flex-col">
          <div className='p-4 border-b border-gray-700'>
